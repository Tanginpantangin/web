...
window.cam.service.TransToCamPaxalih = function () {
  ...
  self.DoConvert = function (data, sourceType, destitionType) {
    ...
    data = data.replace(/(?:\r\n|\r|\n)/g, ' \n ');
    data = window.cam.service.Utility.fixString(data);
    ...
    words.forEach(function (word) {
      ...
      lstAkhar = self.correctTypedOrderForUnicode(lstAkhar, destitionType);
      ...
      for (var i = 0; i < lstAkhar.length; i++) {
        ...
        switch (destitionType) {
          ...
          default:
            if (akhar == Model.AKhar.TakaiThekPaohNgâk) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TakaiThek].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.PaohNgâk].toString();
            } else if (akhar == Model.AKhar.TakaiKlakTakaiKuak) {
            ...
            } else if (akhar == Model.AKhar.TraohAwTutTakaiMâk) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TraohAw].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TutTakaiMâk].toString();
            } else if (akhar == Model.AKhar.TraohAwPaohNgâk) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TraohAw].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.PaohNgâk].toString();
            } else if (akhar == Model.AKhar.TakaiKikTutTakaiMâkLingiw) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TakaiKik].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TutTakaiMâk].toString();
            } else if (akhar == Model.AKhar.TakaiThekTutTakaiMâk) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TakaiThek].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TutTakaiMâk].toString();
            } else if (akhar == Model.AKhar.BalauTapong) {
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.TakaiThek].toString();
              convertedWord += base._keyCodeToUniCamKur[Model.AKhar.Balau].toString();
            } else {
  ...
  self.correctTypedOrderForUnicode = function (akharList, destitionType) {
    ...
    // Takhai sap paoh anak (dar sa, dar dua)
    // Top chars
    // Others
  ...
  self.correctSapPoahAnakForUnicode = function (akharList) {
    ...
    for (var i = 0; i < arrayLength; i++) {
      var currentChar = akharList[i];
      if (!base.Check_TakaiSapPaohAnak(currentChar)) continue;

      result = self.insertAfterNextInaAkhar(i, result);
    }
    return result;
  }

  self.correctTakaiAnakForUnicode = function (akharList) {
    var result = window.cam.service.Utility.CopyListAkhar(akharList);
    var arrayLength = akharList.length;
    for (var i = 0; i < arrayLength; i++) {
      var currentChar = akharList[i];
      if (!base.Check_TakaiAkharAnak(currentChar)) continue;
  ...
  self.insertAfterNextInaAkhar = function (currentIndex, akharList){
    var currentChar = akharList[currentIndex];
    var result = window.cam.service.Utility.CopyListAkhar(akharList);

    var afterInaAkharIndex = base.GetIndexAkharDiipAfter(result, currentIndex);
    afterInaAkharIndex = (afterInaAkharIndex == -1) ? 0 : afterInaAkharIndex;

    result.splice(currentIndex, 1);
    result.splice(afterInaAkharIndex, 0, currentChar);

    return result;
  }

  self.insertAfterPreviousInaAkhar = function (currentIndex, akharList) {
    var currentChar = akharList[currentIndex];
    var result = window.cam.service.Utility.CopyListAkhar(akharList);

    var previousInaAkharIndex = base.GetIndexAkharDiipBefore(result, currentIndex);
    previousInaAkharIndex = (previousInaAkharIndex == -1) ? 0 : previousInaAkharIndex;

    result.splice(currentIndex, 1);
    result.splice(previousInaAkharIndex + 1, 0, currentChar);
  ...
  self.correctTakaiLikukForUnicode = function (akharList) {
    ...
    for (var i = 0; i < arrayLength; i++) {
      var currentChar = akharList[i];
      if (!base.Check_TakaiAkharLikuk(currentChar)) continue;
  ...
  // self.correctInaAkhar = function (akharList) {
  //   var result = window.cam.service.Utility.CopyListAkhar(akharList);
  //   var arrayLength = akharList.length;
  //   for (var i = 0; i < arrayLength; i++) {
  //     var currentChar = akharList[i];
  //     if (!base.IsConsonant(currentChar)) continue;

  //     result = self.insertAfterClosestInaAkhar(i, result);
  //   }
  //   return result;
  // }

  // self.insertAfterClosestInaAkhar = function (currentIndex, akharList){
  //   var currentChar = akharList[currentIndex];
  //   var result = window.cam.service.Utility.CopyListAkhar(akharList);

  //   var beforeCurrentArr = result.slice(0, currentIndex);
  //   var afterInaAkharIndex = base.GetIndexAkharDiip(beforeCurrentArr);
  //   afterInaAkharIndex = (afterInaAkharIndex == beforeCurrentArr.length) ? 0 : (afterInaAkharIndex + 1);
  ...
  // self.correctAfterBottomCharsForUnicode = function (akharList) {
  //   var result = window.cam.service.Utility.CopyListAkhar(akharList);
  //   var arrayLength = akharList.length;
  //   for (var i = 0; i < arrayLength; i++) {
  //     var currentChar = akharList[i];
